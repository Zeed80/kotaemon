    ...
    def detect_task_type(self, query: str) -> str:
        ...
        if any(
            ...
            if any(
                ...
                word in query_lower for word in ["как", "почему", "зачем", "how", "why"]
            ):
                task_type = TaskType.ANALYTICAL_QUERY
            else:
                task_type = TaskType.FACTUAL_QUERY
        elif any(
            word in query_lower for word in ["сущность", "объект", "entity", "object"]
        ):
            task_type = TaskType.ENTITY_EXTRACTION
        elif any(
            word in query_lower
            for word in ["связь", "отношение", "relationship", "relation", "connection"]
        ):
            task_type = TaskType.RELATIONSHIP_EXTRACTION
        elif any(
            word in query_lower
            for word in ["сообщество", "группа", "community", "group", "cluster"]
        ):
            task_type = TaskType.COMMUNITY_ANALYSIS
        elif any(
            word in query_lower
            for word in ["суммар", "кратко", "summary", "summarize", "краткое"]
        ):
...
_prompt_adapter: PromptAdapter | None = None
